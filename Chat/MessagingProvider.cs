...
namespace Chat
{
    public sealed class MessagingProvider : IMessagingProvider, IInitializable, IDisposable
    {
        ...
        public MessagingProvider(string exchangeName, string requestQueueName, IConnection connection)
        {
        ...
        public void Send(Envelope envelope)
        {
            ...
            properties.ReplyTo = envelope.ReplyTo;
            properties.CorrelationId = envelope.CorrelationId;
            properties.Type = envelope.BodyType.ToString();
            _channelProduce.BasicPublish(_exchangeName, envelope.ReplyTo, properties, envelope.Body);
        }
            ...
            _consumerReceivedHandler = (sender, args) =>
            {
                Task.Run(() =>
                {
                    var envelope = new Envelope
                    {
                        ...
                        BodyType = Type.GetType(args.BasicProperties.Type)
                    };
